...
fn main() {
    ...
    */
    ...
    // Lista
    let mut my_list = vec!["Ricardo", "Rosero", "@rrosero2000"];
    my_list.push("Brais");
    println!("{:?}", my_list);
    println!("{}", my_list[1]);

    // Sets
    let mut my_set: HashSet<&str> = vec!["Ricardo", "Rosero", "@rrosero2000"].into_iter().collect();
    my_set.insert("Brais");
    println!("{:?}", my_set);

    // Mapas
    let mut my_map: HashMap<&str, i32> = vec![("Ricardo", 42), ("Mabequily", 45), ("Demegorash", 96)]
        .into_iter()
        .collect();
    ...
    // Estructuras
    let my_struct = MyStruct::new("Ricardo", 42);
